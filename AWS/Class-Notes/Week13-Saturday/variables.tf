...
variable "prometheus_vpc_id" {
  type        = string
  description = "VPC ID for Prometheus in us-east-2 (Ohio)"
}

variable "prometheus_subnet_id" {
  type        = string
  description = "Subnet ID for Prometheus VM in Ohio"
}

variable "prometheus_ami_id" {
  type        = string
  description = "AMI ID for Prometheus VM in Ohio region"
}

variable "prometheus_admin_cidr" {
  type        = string
  description = "Your IP for SSH/UI access in Ohio"
}
variable "prometheus_vpc_cidr" {
  description = "CIDR block of the Prometheus VPC in Ohio (used for SG rules)"
  type        = string
}
