...
# Allow ping from Ohio (Prometheus VPC CIDR)
resource "aws_vpc_security_group_ingress_rule" "targets_icmp_from_prometheus" {
  ...
  to_port           = -1
  ip_protocol       = "icmp"
  description       = "ICMP ping from Prometheus VPC (Ohio)"
}

# Allow Prometheus (Ohio) to scrape node_exporter on 9100
resource "aws_vpc_security_group_ingress_rule" "targets_9100_from_prometheus" {
  ...
  to_port           = 9100
  ip_protocol       = "tcp"
  description       = "Node exporter from Prometheus (Ohio)"
}
